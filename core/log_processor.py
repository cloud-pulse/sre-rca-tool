...
class LogProcessor:
    ...
    def __init__(self):
        """Initialize and pre-compile regular expressions for performance optimization."""
        # Timestamp regexes
        ...
        self._re_levels = [(level, re.compile(r'\b' + level + r'\b', re.IGNORECASE)) for level in self.VALID_LEVELS]
        self._re_aliases = [(alias, re.compile(r'\b' + alias + r'\b', re.IGNORECASE)) for alias in ['crit', 'warning']]

        # Service regexes
        self._re_bracket_service = re.compile(r'\[([a-z\-]+)\]')
        self._re_services = [(service, re.compile(r'\b' + re.escape(service) + r'\b', re.IGNORECASE)) for service in self.KNOWN_SERVICES]
        ...
        self._re_msg_services = [re.compile(r'\b' + re.escape(service) + r'\b\s*', re.IGNORECASE) for service in self.KNOWN_SERVICES]
    ...
    def _extract_level(self, line: str) -> str:
        ...
        for valid_level, re_lvl in self._re_levels:
            if re_lvl.search(line):
                return valid_level

        # Check for aliases
        for alias_lower, re_alias in self._re_aliases:
    ...
    def _extract_service(self, line: str) -> str:
        ...
        for service, re_svc in self._re_services:
    ...
    def _extract_message(self, line: str) -> str:
        ...
        for re_lvl in self._re_msg_levels:
            message = re_lvl.sub('', message)

        # Remove service names
        for re_svc in self._re_msg_services:
