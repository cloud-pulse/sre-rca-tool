...
class LogProcessor:
    ...
    def __init__(self):
        """Initialize the LogProcessor and pre-compile regular expressions."""
        # Timestamp patterns
        ...
        self.re_valid_levels = [re.compile(r'\b' + valid_level + r'\b', re.IGNORECASE) for valid_level in self.VALID_LEVELS]
        self.re_alias_levels = {alias_lower: re.compile(r'\b' + alias_lower + r'\b', re.IGNORECASE) for alias_lower in ['crit', 'warning']}
        ...
        self.re_msg_time_timestamp = re.compile(r'\d{1,2}:\d{2}(?::\d{2})?\s*')
        self.re_msg_levels = [re.compile(r'\b' + level + r'\b\s*', re.IGNORECASE) for level in ['ERROR', 'CRITICAL', 'CRIT', 'WARN', 'WARNING', 'INFO', 'DEBUG']]
        self.re_msg_services = [re.compile(r'\b' + re.escape(service) + r'\b\s*', re.IGNORECASE) for service in self.KNOWN_SERVICES]
    ...
    def _extract_level(self, line: str) -> str:
        ...
        for i, valid_level in enumerate(self.VALID_LEVELS):
            if self.re_valid_levels[i].search(line):
                return valid_level

        # Check for aliases
        for alias_lower in ['crit', 'warning']:
            if self.re_alias_levels[alias_lower].search(line):
    ...
    def _extract_service(self, line: str) -> str:
        ...
        for i, service in enumerate(self.KNOWN_SERVICES):
            if self.re_known_services[i].search(line):
    ...
    def _extract_message(self, line: str) -> str:
        ...
        message = self.re_msg_time_timestamp.sub('', message)
