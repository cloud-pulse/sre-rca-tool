...
class LogProcessor:
    ...
    def __init__(self):
        # ⚡ Bolt: Pre-compile regular expressions to optimize log processing performance.
        # This significantly speeds up the _extract_timestamp, _extract_level,
        # _extract_service, and _extract_message methods which are called per-line.
        ...
        for level in ['ERROR', 'CRITICAL', 'CRIT', 'WARN', 'WARNING', 'INFO', 'DEBUG']:
            self._message_sub_patterns.append(re.compile(r'\b' + level + r'\b\s*', flags=re.IGNORECASE))

        for service in self.KNOWN_SERVICES:
            self._message_sub_patterns.append(re.compile(r'\b' + re.escape(service) + r'\b\s*', flags=re.IGNORECASE))

        self._message_clean_pattern = re.compile(r'^[\s\-]+')
    ...
    def _extract_level(self, line: str) -> str:
        ...
        for valid_level, pattern in self._valid_level_patterns:
    ...
    def _extract_service(self, line: str) -> str:
        ...
        for service, pattern in self._known_service_patterns:
            if pattern.search(line):
    ...
    def _extract_message(self, line: str) -> str:
        ...
        for pattern in self._message_sub_patterns:
            message = pattern.sub('', message)

        # Remove leading punctuation and dashes
        message = self._message_clean_pattern.sub('', message)
